...
# - /home/user/SWPJ3/auto-subtitle-service/ai/data/results/game_lora/adapter
#
# 출력:
# - /home/user/SWPJ3/auto-subtitle-service/ai/data/results/game_lora/val_json/game_validation_predictions.json
# - /home/user/SWPJ3/auto-subtitle-service/ai/data/results/game_lora/val/game_validation_metrics.json
#
# 평가 지표:
...
#
# 목적:
# - 학습이 끝난 game adapter가 validation 세트에서 어느 정도 성능을 내는지 확인한다.
...
def normalize_text(text: str) -> str:
    ...
    목적:
    - reference와 prediction 비교 시
      공백 차이 때문에 WER/CER가 과하게 흔들리는 것을 줄인다.
    """
    text = text.strip()
...
def main():
    """
    validation 전체 평가를 수행하는 메인 함수.

    전체 흐름:
    1. validation.jsonl 로드
    2. base Whisper + 저장된 adapter 로드
    3. validation 전체 샘플 추론
    4. 샘플별 결과(results) 저장
    5. 전체 평균 지표(metrics) 계산 및 저장
    """
    ...
    for idx, sample in enumerate(tqdm(val_dataset, desc="Validation inference")):
        ...
        # forward loss 계산
        # -------------------------
        # Whisper 허용 길이 초과 샘플은 loss만 건너뛴다.
        sample_loss = None
        loss_skipped = False
