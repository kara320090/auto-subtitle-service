# ============================================
# 파일명: test_adapter_reload_with_timestamps_vacation_all_train.py
#
# 역할:
# - vacation 도메인용으로 학습된 LoRA adapter를 다시 불러온다.
# - whisper-large-v3 base 모델에 vacation adapter를 붙인다.
# - raw/vacation/train 안의 모든 .wav 파일을 순회하며 추론한다.
...
RAW_TRAIN_DIR = PROJECT_ROOT / "ai/data/raw/vacation/train"
RESULT_DIR = PROJECT_ROOT / "ai/data/results/vacation_lora"
ADAPTER_DIR = RESULT_DIR / "adapter"
PRED_DIR = RESULT_DIR / "predictions_train_all"
...
# =========================
# train 폴더 안 wav 전부 수집
# =========================
if not RAW_TRAIN_DIR.exists():
    raise FileNotFoundError(f"train directory not found: {RAW_TRAIN_DIR}")

audio_files = sorted(RAW_TRAIN_DIR.glob("*.wav"))

if not audio_files:
    raise FileNotFoundError(f"no wav files found in: {RAW_TRAIN_DIR}")
...
# 전체 결과 통합 저장
# =========================
all_out_path = PRED_DIR / "all_predictions_with_timestamps.json"
with all_out_path.open("w", encoding="utf-8") as f:
    json.dump(all_results, f, ensure_ascii=False, indent=2)

print("\n" + "=" * 60)
print(f"[INFO] all predictions saved to: {all_out_path}")